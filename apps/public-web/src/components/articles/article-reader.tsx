    ...
    <article className="mx-auto max-w-5xl space-y-8">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-3 py-1.5 text-[10px] font-bold text-[#1B1712] transition hover:bg-slate-200"
            onClick={() => {
              void handleShare();
            }}
            type="button"
      ...
      <header className="space-y-5">
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-md bg-[#1B1712] px-3 py-1 text-[10px] font-bold text-white">
        ...
        <h1 className="max-w-4xl font-serif text-4xl font-black leading-[1.05] tracking-tight text-[#1B1712] sm:text-5xl lg:text-6xl">
          {article.title}
        </h1>
        ...
        <div className="flex flex-col gap-4 border-y border-slate-200 py-4 text-xs sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <strong className="block text-sm text-[#1B1712]">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10 lg:p-12">
          ...
          {paragraphs.length > 0 ? (
            paragraphs.map((paragraph, index) => (
              <p
                className={
                  index === 0
                    ? "first-letter:float-left first-letter:mr-2 first-letter:font-serif first-letter:text-5xl first-letter:font-black first-letter:leading-none first-letter:text-[#1B1712]"
                    : undefined
                }
                key={index}
              >
        ...
        <div className="mx-auto mt-10 flex max-w-3xl flex-col gap-3 border-t border-slate-200 pt-6 sm:flex-row sm:items-center sm:justify-between">
          <Link
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#1B1712] px-5 text-xs font-bold text-white transition hover:bg-[#0F0C09]"
            href={`/n/${article.issue_nfc_slug}?src=web`}
          >
            <Icon
              className="text-[#D9622B]"
